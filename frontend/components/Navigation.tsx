...
export default function Navigation({ initialLocale = 'he' }: NavigationProps) {
  ...
  const handleLanguageChange = (newLang: string) => {
    setLanguage(newLang)
    localStorage.setItem('studymaterials-language', newLang)
    ...
    if (segments[0] && Object.keys(LANGUAGES).includes(segments[0])) {
      ...
      // Just go to the new locale root
      router.push(`/${newLang}`)
    }
  }

  return (
    <header className="bg-white border-b border-gray-200 sticky top-0 z-50" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="max-w-7xl mx-auto px-6 py-3">
        <div className="flex items-center justify-between">
          {/* Logo and Site Name */}
          <a href={`/${language}`} className="flex items-center gap-0 hover:opacity-80 transition-opacity">
